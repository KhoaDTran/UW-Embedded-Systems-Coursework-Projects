...
void pageChange(TouchScreen ts, TSPoint p, int* page) {
    ...
    *****************/
    ...
    if (p.z > ts.pressureThreshhold) {
        ...
        if (p.x >= 8 && p.x <= 58 && p.y >= 255 && p.y <= 300) {
            *page = MEASURE_PAGE;
        // If you press the alarm button, switch to the alarm page
        } else if (p.x >= 66 && p.x <= 116 && p.y >= 255 && p.y <= 300) {
            *page = ALARM_PAGE;
        // If you press the battery button, switch to the battery on/off page
        } else if (p.x >= 124 && p.x <= 174 && p.y >= 255 && p.y <= 300) {
            *page = BATTERY_PAGE;
        // If you press the accelerometer button, switch to the accelerometer page
        } else if (p.x >= 182 && p.x <= 232 && p.y >= 255 && p.y <= 300) {
            *page = ACCELEROMETER_PAGE;
        }
    }
}
