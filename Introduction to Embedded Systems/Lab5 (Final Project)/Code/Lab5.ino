...
// Set the system's time base
unsigned long timeBaseInMs = 50;
unsigned long timeBaseInUs = timeBaseInMs * 1000;
...
int page              = ACCELEROMETER_PAGE;                   // The current page the system is displaying. Start at the measurement page
...
// Accelerometer Data
accelerometerData accelerometer;                        // Declare accelerometer data structure - defined in Accelerometer.h
const byte xPin   = A13;                                // The pin the system is reading to see the acceleration in the x direction
const byte yPin   = A14;                                // The pin the system is reading to see the acceleration in the y direction
const byte zPin   = A15;                                // The pin the system is reading to see the acceleration in the z direction
float timeBase    = timeBaseInMs / 1000.0;              // The system's time base in seconds
float currXPos    = 0;                                  // The current x position of the accelerometer in cm
float currYPos    = 0;                                  // The current y position of the accelerometer in cm
float currZPos    = 0;                                  // The current z position of the accelerometer in cm
float currXVel    = 0;                                  // The current x velocity of the accelerometer in cm
float currYVel    = 0;                                  // The current y velocity of the accelerometer in cm
float currZVel    = 0;                                  // The current z velocity of the accelerometer in cm
float currXAcc    = 0;                                  // The current x acceleration of the accelerometer in cm
float currYAcc    = 0;                                  // The current y acceleration of the accelerometer in cm
float currZAcc    = 0;                                  // The current z acceleration of the accelerometer in cm
float totalDist   = 0;                                  // The total distance travelled of the accelerometer in cm
float currXAng    = 0;                                  // The current x angle w.r.t. gravity of the accelerometer in cm
float currYAng    = 0;                                  // The current y angle w.r.t. gravity of the accelerometer in cm
float currZAng    = 0;                                  // The current z angle w.r.t. gravity of the accelerometer in cm

// Set up variables for the dynamic scheduler
const int timeBasePeriod      = timeBaseInMs;
...
void setup() {
  ...
  displayD                      = {&measure, &soc, &alarm, &contactor, &accelerometer, &page, &oldPage, &tft, &alarmButtonDrawn};
  ...
  // Initilize the accelerometer data structure with the correct values
  accelerometer                 = {&xPin, &yPin, &zPin, &timeBase, &currXPos, &currYPos, &currZPos, &currXVel, &currYVel, &currZVel, &currXAcc,
                                   &currYAcc, &currZAcc, &totalDist, &currXAng, &currYAng, &currZAng};
  ...
  accelerometerTCB.period       = &timeBasePeriod;
  accelerometerTCB.timeRemain   = &accelerometerTimeRemain;
  ...
  // Initialize the timer to run at the system time base
  Timer1.initialize(timeBaseInUs);
...
void buildQueue() {
  ...
  *currTCB->timeRemain -= timeBaseInMs;
  }
}
