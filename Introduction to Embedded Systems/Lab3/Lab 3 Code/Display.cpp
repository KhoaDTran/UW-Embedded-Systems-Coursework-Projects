...
void measurePage(Elegoo_TFTLCD tft, bool newScreen, measurementData mData, socData sData) {
    ...
    *****************/
    ...
    if (newScreen) {
        ...
        // Update the SOC State if we just switched to this screen
    ...
    // Update the Temperature State Change --- if the temperature changed or if we just switched to this screen
    ...
    // Update the Current State Change --- if the current changed or if we just switched to this screen
    ...
    // Update the Voltage State Change --- if the voltage changed or if we just switched to this screen
...
void alarmPage(Elegoo_TFTLCD tft, bool newScreen, alarmData aData, bool* buttonDrawn) {
    ...
    *****************/
    ...
    if (*aData.hvilAlarmChanged || newScreen) {
        ...
        tft.setCursor(95, 58);
        Serial.println(*aData.hvilAlarmStatus);
