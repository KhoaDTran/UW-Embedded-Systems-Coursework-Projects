...
void* mm_malloc(size_t size) {
  ...
  BlockInfo * nextBlockPtr = NULL;
  size_t targetSize = NULL;
  size_t remainBlockSize = NULL;
  size_t *footerPtr = NULL;
  size_t remainFreeBlock = NULL;
  ...
  //Get pointer in free list that fits reqSize
  ptrFreeBlock = searchFreeList(reqSize);
  //If there wasn't a block in free list for reqSize,
  //request more space and get pointer
  if (ptrFreeBlock == NULL) {
    requestMoreSpace(reqSize);
    ptrFreeBlock = searchFreeList(reqSize);
  }
  //get preceding used tag
  precedingBlockUseTag = (*ptrFreeBlock).sizeAndTags & TAG_PRECEDING_USED;
  //get block size
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  removeFreeBlock(ptrFreeBlock);
  targetSize = blockSize - reqSize;
  //if targetBlock size is larger than minimum block size, split block
  if (targetSize >= MIN_BLOCK_SIZE) {
    //set size and tag
    (*ptrFreeBlock).sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED;
    //add remaining free block
    nextBlockPtr = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    
    remainBlockSize = blockSize - WORD_SIZE;
    //boundary tag of remaining free block
    footerPtr = (size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, remainBlockSize);
    
    remainFreeBlock = blockSize - reqSize;
    //set preceding used tag in remaining free block
    (*nextBlockPtr).sizeAndTags = (size_t)(remainFreeBlock | TAG_PRECEDING_USED);
    
    //set preceding used tag in boundary tag
    (*footerPtr) = (size_t)(remainFreeBlock | TAG_PRECEDING_USED);
    
    //insert remaining free block to free list
    insertFreeBlock(nextBlockPtr);
  } else { //don't split, add to following block
    //set used tag
...
void mm_free(void *ptr) {
  ...
  //set following block size and tags to preceding used tag
  (*followingBlock).sizeAndTags = (size_t) ((*followingBlock).sizeAndTags & ~TAG_PRECEDING_USED);

  //set block size and tags to tag used
  (*blockInfo).sizeAndTags = (size_t) (*blockInfo).sizeAndTags & (~TAG_USED);
  footerSize = payloadSize - WORD_SIZE;
  //set footer 
  footerPtr = (size_t*) UNSCALED_POINTER_ADD(blockInfo, footerSize);
  //set footer size and tags 
  *footerPtr = (size_t) (*blockInfo).sizeAndTags & (~TAG_USED);
  //insert block to free list
  insertFreeBlock(blockInfo);
  //coalesce the following free blocks with preceding
